...
trait TransactionGenBase extends ScriptGen {
  ...
  def modernLeaseCancelLeaseGen: Gen[(LeaseTx, LeaseCancelTx)] = {
    for {
      ...
      lv <- Gen.oneOf(LeaseTx.supportedVersions.toSeq)
      lcv <- Gen.oneOf(LeaseCancelTx.supportedVersions.toSeq)
      lh <- txHeaderGen(sender, LeaseTx.typeId, lv)
      lch <- txHeaderGen(sender, LeaseCancelTx.typeId, lcv)
      proofs <- proofsGen
    } yield {
      val lpl = LeasePayload(amount, recipient)
      val leaseTx = LeaseTx(lh, lpl, proofs)

      val lcpl = LeaseCancelPayload(leaseTx.leaseId())
      val leaseCancelTx = LeaseCancelTx(lch, lcpl, proofs)
  ...
  def setScriptTxGen: Gen[SetScriptTx] = {
    for {
      ...
      payload <- scriptGen.map(s => SetScriptPayload(AddressScheme.current.chainId, s))
      proofs <- proofsGen
